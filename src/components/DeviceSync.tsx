...
export default function DeviceSync() {
  ...
  return (
    <section id="devices" className="py-24 bg-white relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="flex flex-col lg:flex-row lg:items-end justify-between mb-16 gap-6">
          ...
          <motion.button
            ...
            className="px-6 py-3 rounded-2xl bg-brand-dark hover:bg-slate-800 text-white font-sans font-bold text-xs flex items-center justify-center gap-2 shadow-lg shadow-slate-900/10 cursor-pointer self-start lg:self-end"
          >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          ...
          <div className="lg:col-span-5 flex items-center justify-center relative py-8">
            ...
            <div className="absolute w-[240px] h-[240px] rounded-full border border-brand-sky/20 animate-pulse-slow" />
            <div className="absolute w-[340px] h-[340px] rounded-full border border-brand-sky/10 animate-ping opacity-25" style={{ animationDuration: '4s' }} />
            <div className="absolute w-[140px] h-[140px] rounded-full bg-brand-sky/5 filter blur-xl" />

            {/* Pulsing Beacon Circle */}
            <div className="w-24 h-24 rounded-full bg-brand-sky/10 border border-brand-sky/30 flex items-center justify-center relative shadow-lg shadow-brand-sky/5 z-10 animate-float-slow">
              ...
              <div className="absolute -bottom-2 -right-2 px-2.5 py-0.5 rounded-lg bg-slate-900 border border-white/10 text-[9px] font-mono text-white flex items-center gap-1">
          ...
          <div className="lg:col-span-7 grid grid-cols-1 sm:grid-cols-2 gap-6 w-full">
            {devices.map((device, idx) => (
              <motion.div
                ...
                className="p-5 rounded-3xl glass-card border border-slate-200/60 shadow-sm hover:shadow-md transition-all duration-300 flex items-center justify-between gap-4"
              >
                <div className="flex items-center gap-3.5 min-w-0">
                  ...
                  <div className="min-w-0">
                    <h3 className="font-display font-bold text-sm text-brand-dark truncate">
                      {device.name}
                    </h3>
                    
                    <div className="flex items-center gap-2 mt-1">
                      {getStatusBadge(device.status)}
                      <span className="text-[10px] text-slate-400 font-mono">• {device.lastSync}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex flex-col items-end shrink-0">
                  ...
                  <span className="text-[9px] text-slate-400 uppercase tracking-widest mt-0.5 font-bold">BATTERY</span>
                </div>
              </motion.div>
            ))}
          </div>
